...
class ShipyardDataGeneratorTest(unittest.TestCase):
    def test_empirical_profile_roundtrip_preserves_exact_generation(self) -> None:
        ...
        fitted = module.ShipyardGenerator(
            root / "input" / "260724_절단WO_데이터_None.xlsx",
            root / "input" / "260724_절단블록_데이터_None.xlsx",
            mode="spearman",
            series="FN",
        ).fit()
    ...
    def test_fl_cut_first_block_chain_is_fitted_as_length_to_cut_to_mark(self) -> None:
        module = _load_generator_module()
        length = np.arange(100.0, 700.0, 100.0)
        cut = 5.0 * length + 7.0
        mark = 3.0 * cut + 2.0
        wo_quantity = 0.25 * cut + 1.0
        blocks = pd.DataFrame(
            {
                "LTH": length,
                "MARK_LTH": mark,
        ...
        parameters = module._fit_block_chain_parameters(
            blocks,
            chain_order="cut_first",
        )

        self.assertAlmostEqual(parameters["cut_a"], 5.0)
        self.assertAlmostEqual(parameters["cut_b"], 7.0)
        self.assertAlmostEqual(parameters["mark_a"], 3.0)
        self.assertAlmostEqual(parameters["mark_b"], 2.0)
        self.assertAlmostEqual(parameters["wo_count_a"], 0.25)
        self.assertAlmostEqual(parameters["wo_count_b"], 1.0)

    def test_fl_chain_and_dirichlet_preserve_block_mark_sum(self) -> None:
        module = _load_generator_module()
        root = Path(__file__).parents[1]
        generator = module.ShipyardGenerator(
            root / "input" / "260724_절단WO_데이터_None.xlsx",
            ...
            mode="pearson",
            series="FL",
            fl_mark_method="chain",
        ).fit()

        self.assertEqual(generator.mark_aggregation, "sum")
        self.assertIsNotNone(generator.fl_dirichlet_alpha_coef)
        for method in module.FL_MARK_METHODS:
            generator.fl_mark_method = method
            work_orders, blocks = generator.generate(
                n_blocks=3,
                seed=20260724,
                wo_counts=(3, 4, 2),
            ...
            generated_sum = work_orders.groupby("BLK_ID", sort=True)["MARK_LTH"].sum()
            expected_sum = blocks.sort_values("BLK_ID")["MARK_LTH"]
            np.testing.assert_allclose(
                generated_sum.to_numpy(dtype=float),
                expected_sum.to_numpy(dtype=float),
                rtol=0.0,
                atol=1e-6,
            )

            restored = module.ShipyardGenerator.from_generation_profile(
                generator.to_generation_profile()
            )
            actual_work_orders, actual_blocks = restored.generate(
                n_blocks=3,
                seed=20260724,
                wo_counts=(3, 4, 2),
                block_seeds=(101, 202, 303),
            )
            assert_frame_equal(work_orders, actual_work_orders, check_exact=True)
            assert_frame_equal(blocks, actual_blocks, check_exact=True)

    def test_invalid_fl_mark_method_fails_before_reading_input(self) -> None:
        module = _load_generator_module()
        with patch.object(module.pd, "read_excel") as read_excel:
            with self.assertRaisesRegex(RuntimeError, "unsupported_fl_mark_method"):
                module.ShipyardGenerator(
                    "wo.xlsx",
                    "block.xlsx",
                    series="FL",
                    fl_mark_method="unknown",
                )
        read_excel.assert_not_called()
