...
`Utils.phase1.phase1_mdp` and use cross-entropy on the selected action index.
"""
...
class Phase1PairPointerPolicy(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # LINE-BY-LINE: pair 후보 feature 차원이 유효한지 검사합니다. 예: 현재 Phase 1 pair feature는 graph edge 10차원입니다.
        _require_positive_dim(pair_feature_dim, "pair_feature_dim")
        # LINE-BY-LINE: 환경 feature 차원이 유효한지 검사합니다. 예: 현재 Phase 1 env feature는 5차원입니다.
        _require_positive_dim(env_feature_dim, "env_feature_dim")
