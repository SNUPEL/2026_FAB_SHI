...
def run_phase1_policy_rollout(
    ...
    bay_loads = _empty_phase1_bay_loads(normalized_bay_ids)
...
def _run_phase1_greedy_bank_candidate(
    ...
    bay_loads = _empty_phase1_bay_loads(tuple(bay_ids), bay_capacity_weights)
    assignments: Dict[str, str] = {}
    for block in blocks:
        selected_bay = min(
...
def _projected_bank_score(
    ...
    if bay_score_rule == "multi_objective":
        # PPB: after tentative insertion, minimize global Bay gaps in business priority order.
        return _multi_objective_load_score(projected, score_mode="steel_first") + (str(bay_id),)
    if bay_score_rule == "long_cut_preferred":
        # LCP: Bay24 long-cut avoidance is handled by candidate hard masking.
        steel_gap, cut_gap, bevel_gap = _multi_objective_load_score(
            projected,
