...
| Phase | 1 |
| 담당 (`--owner`) | **YJ** |
| 원 학습 경로 | `output\phase1_anneal_v1` |
| 캠페인 / 비교 축 | `temperature`: P1-BASE 대비 **1.0→0.3 annealing** |
| 학습 상태 | ✅ 20,000 ep 완주 — **지금 제출 대상** |

로컬에 checkpoint 2개(ep200, ep19800)와 CSV 사본이 이미 있으나 **부분 사본**이다. 완전한 1차 제출로 대체한다.
...
best-rate 상위 후보 3개를 지목하면, 그 `.pt`만 `checkpoints/P1-ANNEAL_ep<NNNNN>.pt`로 넣는다.
개당 약 1.1 MB.
...
du -sh experiments/p1_temperature/P1-ANNEAL          # 50 MB 이하여야 한다
```
