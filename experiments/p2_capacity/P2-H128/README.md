...
| Phase | 2 |
| 담당 (`--owner`) | **(로컬)** |
| 원 학습 경로 | `output/phase2_h128` |
| 캠페인 / 비교 축 | `hidden_dim`: P2-BASE(h16) 대비 **h128** |
| 학습 상태 | ✅ 20,000 ep 완주 |

이 머신 `output/`에 원본이 있다. arm identity `phase2_capacity / hidden_dim / h128`가 기록된 유일한 run이다.
...
best-rate 상위 후보 3개를 지목하면, 그 `.pt`만 `checkpoints/P2-H128_ep<NNNNN>.pt`로 넣는다.
...
du -sh experiments/p2_capacity/P2-H128          # 50 MB 이하여야 한다
```
