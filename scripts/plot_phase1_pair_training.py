...
def _subproblem_sources(source: str) -> list[str]:
    """parent best_source를 subproblem별 후보 이름으로 나눈다.

    resource_pool_subproblems_v1은 'NP_NC:<source>|FN_FL:<source>' 형태로 쓰고,
    단일 subproblem 구형 실행은 후보 이름을 그대로 쓴다.
    """
...
def _quick_status(
    ...
    subproblem_sources = [
        candidate for source in sources for candidate in _subproblem_sources(source)
    ]
    ...
    recent_subproblem_sources = [
        candidate for source in recent for candidate in _subproblem_sources(source)
    ]
    recent_agent_count = sum(
        1 for candidate in recent_subproblem_sources if candidate.startswith("agent_")
    )
    if not subproblem_sources or not recent_subproblem_sources:
        print("[ERROR][plot_phase1_pair_training._quick_status] cause=empty_best_source")
