...
이 파일의 책임:
- `Utils/cutting_data_loader.py`가 정리한 records를 환경 입력 scenario dict로 바꾼다.
...
# LINE-BY-LINE: `.tact_time` 모듈에서 `CORE_TACT_FEATURE_KEYS, fit_actual_duration_formula, predict_actual_duration_from_formula`를 가져옵니다. 사용: 이 파일의 타입 생성/함수 호출에 직접 씁니다.
from .tact_time import CORE_TACT_FEATURE_KEYS, fit_actual_duration_formula, predict_actual_duration_from_formula
