...
def _as_tuple(value: Any, field_name: str) -> Tuple[str, ...]:
    ...
    if isinstance(value, str):
        ...
        result = ()
    if not result:
        print(f"[ERROR][Utils.data.io._as_tuple] cause=empty field={field_name} input={value}")
...
def _filter_records_for_process_time_source(cleaned: Any, process_time_source: str) -> Any:
    ...
    if process_time_source != "tact_time":
        print(
            "[CHECK][Utils.data.io._filter_records_for_process_time_source] "
    ...
    for record in cleaned.records:
        ...
        try:
            ...
            print(
                "[ERROR][Utils.data.io._filter_records_for_process_time_source] "
...
def _load_scenario_from_source_data(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not source_data_path:
        print("[ERROR][Utils.data.io._load_scenario_from_source_data] cause=missing paths.source_data_path")
    ...
    if not process_time_source_value:
        print(
            "[ERROR][Utils.data.io._load_scenario_from_source_data] "
    ...
    print(
        "[CHECK][Utils.data.io._load_scenario_from_source_data] "
...
def load_scenario_for_config(
    ...
    if scenario_path:
        ...
        if not path.exists():
            print(
                "[ERROR][Utils.data.io.load_scenario_for_config] "
