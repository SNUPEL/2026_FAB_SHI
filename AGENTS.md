...
- 입력 실적 데이터는 전처리본 `input/260724_절단블록_데이터_None.xlsx`와 `input/260724_절단WO_데이터_None.xlsx` 두 개뿐이다. 다른 원천 파일을 추가하지 않는다.
- 두 파일은 NP/FN/FL/NC 4계열을 모두 포함한다. 블록 1,137건, W/O 6,644건이다.
- `config_mixed.yaml`은 Phase 2 제약 profile만 제공한다. 데이터 경로와 공장 topology를 지정하지 않는다.
- 학습 입력은 config가 아니라 `Utils/data/multi_series_generation_profile.json`에서 온다. 실행 중 Excel을 다시 적합하지 않는다.
...
```bash
python3 -X pycache_prefix=/tmp/pmsp_pycache -m py_compile main.py Phase1/*.py Phase2/*.py Environment/*.py Environment/constraints/*.py Utils/data/*.py Utils/learning/*.py Utils/phase1/*.py Train/network/*.py scripts/*.py
python3 -m pytest tests/ -q
python3 main.py generate-phase1-blocks --n-blocks 40 --seed 0 --output-dir output/_check/blocks
python3 main.py phase1-plan-multi-series --block-xlsx input/260724_절단블록_데이터_None.xlsx --wo-xlsx input/260724_절단WO_데이터_None.xlsx --output-dir output/_check/plan
git diff --check
```

학습 경로까지 확인할 때는 §12 아래의 짧은 Phase 1/2 학습·full-flow 명령을 추가로 실행한다.

확인 기준:

- pytest 전건 통과
- `generate-phase1-blocks`가 NP/FN/FL/NC 4계열을 모두 생성
- `phase1-plan-multi-series`가 블록 1,137건과 W/O 6,644건을 읽고 problem 58건 생성
- Phase 1 학습 로그에 `resource_pool_subproblems_v1`과 `NP_NC`/`FN_FL` 두 subproblem update가 보임
- full-flow `constraint_violation_count`가 0
